...
**Next kernels (planned order)**: compositor blend/opacity/transform per layer → scrub-frame cache and downscaler → audio mixer (gain, fades, pan) → Auto Cut pre-processing (resample, framing) → WebCodecs decode pipeline feeding frames straight into core memory.
